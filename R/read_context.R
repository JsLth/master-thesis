...
read_ioer <- function(indicator, time = "latest", scale = "krs") {
  ...
  feature_types <- req_url_query(req, request = "GetCapabilities") %>%
    ...
    resp_body_xml() %>%
    xml2::as_list() %>%
    extract2("WFS_Capabilities") %>%
    extract2("FeatureTypeList")
  typeName <- vapply(
    feature_types,
    \(x) x$Name[[1]],
...
inkar_ref <- function(database = "data/context/inkar_2021") {
  ...
  ref <- readxl::read_xlsx(
    ...
    select(ags = "Kreise Kennziffer", gen = "Kreise Name") %>%
...
cleanup_inkar <- function(ind, include_gen = TRUE) {
  ...
  ind %>%
    rename(gen = Name) %>%
    select(ags, gen, contains(name)) %>%
    when(!include_gen ~ select(., !gen), ~.) %>%
