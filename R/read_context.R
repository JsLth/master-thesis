...
# Collect IÖR data ----
if (askYesNo("Read IÖR data? (takes about a minute)")) {
...
if (askYesNo("Read INKAR data? (takes about fifteen minutes)")) {
  ...
  inkar_data_join$under_30 <- inkar_data_join$under_30 +
    inkar_data_join$under_25 +
    inkar_data_join$under_18 +
    inkar_data_join$under_6
  inkar_data_join[c("under_25", "under_18", "under_6")] <- NULL
...
context_data <- left_join(
  inkar_data_join %>% dplyr::select(-gen),
  ioer_data_join %>% dplyr::select(-gen),
  by = "ags"
) %>%
  left_join(amenity_data_join, by = "ags") %>%
  dplyr::select(c(ags, names(var_sel)))
